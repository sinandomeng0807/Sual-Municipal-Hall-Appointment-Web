<?php
session_start();
if (!isset($_SESSION['employee_id'])) {
    header("Location: index.html");
    exit();
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Appointment List</title>
    <link rel="stylesheet" href="css/resident-request.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.2/css/all.min.css">
    
</head>
<body>
    <div class="sidebar">
        <div class="logo">
            <img src="Photos/logo.png" alt="Logo">
        </div>
            <ul class="menu">
                <li><a href="dashboard.php"><i class="fa-solid fa-gauge mr-3"></i> Dashboard</a></li>
                <li><a href="appointment-list.php"><i class="fa-solid fa-list mr-3"></i> Appointment List</a></li>
                <li><a href="manage-appointment.php"><i class="fa-solid fa-list-check"></i> Manage Appointment</a></li>
                <li><a href="add-appointment.php"><i class="fa-solid fa-plus mr-3"></i> Add Appointment</a></li>
            </ul>
            <div class="sign-out"onclick="window.location.href='database-connection/logout.php'"><i class="fa-solid fa-arrow-right-from-bracket"></i> Sign Out</div>
        </div>
    <div class="main-container">
        <div class="header">
            <h1>Sual Municipal Hall Admin Panel</h1>
            <div class="user">
                <span><?php echo $_SESSION['employee_id']; ?></span>
                <div class="user-icon"></div>
            </div>
        </div>
            <div class="content-form">
                <div class="form-container">
                    <div class="appointment-type">Resident</div>
                    <div class="form-group">
                        <label>Full Name</label>
                        <input type="text" value="Enter Full Name" readonly>
                    </div>
                    <div class="form-group">
                        <label>Date</label>
                        <input type="date" value="Enter Full Name" readonly>
                    </div>
                    <div class="form-group">
                        <label>Time</label>
                        <input type="time" value="Enter Full Name" readonly>
                    </div>
                    <div class="form-group">
                        <label>Address</label>
                        <input type="text" value="Enter Full Name" readonly>
                    </div>
                    <div class="form-group">
                        <label>Email Address</label>
                        <input type="email" value="Enter Full Name" readonly>
                    </div>
                    <div class="form-group">
                        <label>Barangay</label>
                        <input type="text" value="Enter Full Name" readonly>
                    </div>
                    <div class="form-group">
                        <label>Contact No</label>
                        <input type="text" value="Enter Full Name" readonly>
                    </div>
                    <div class="form-group">
                        <label>Office</label>
                        <input type="text" value="Enter Full Name" readonly>
                    </div>
                    <div class="form-group">
                        <label>Purpose Of Visit</label>
                        <input type="text" value="Enter Full Name" readonly>
                    </div>
                    <div class="form-group">
                        <label>Front ID</label>
                        <div class="image-upload">Upload Front ID</div>
                    </div>
                    <div class="form-group">
                        <label>Back ID</label>
                        <div class="image-upload">Upload Back ID</div>
                    </div>
                    <div class="form-group">
                        <label>Selfie</label>
                        <div class="image-upload">Upload Selfie</div>
                    </div>
                    <div class="button-group">
                        <button class="accept-btn">Accept Request</button>
                        <button class="reject-btn">Reject Request</button>
                    </div>
                </div>
            </div>
        </div>
    </body>
    </html>
    
    